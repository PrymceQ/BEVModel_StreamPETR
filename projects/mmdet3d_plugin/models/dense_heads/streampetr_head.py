...
class StreamPETRHead(AnchorFreeHead):
    ...
    def forward(self, memory_center, img_metas, topk_indexes=None,  **data):
        ...
        x = data['img_feats']
        B, N, C, H, W = x.shape
        num_tokens = N * H * W  # 6*16*44 = 4224
        # memory记录每一个token的256-dim向量
    ...
    def loss_single(self,
        ...
        """
        # 对应batch_size 意味着某个cam下的图片数量
        num_imgs = cls_scores.size(0) # num_imgs = batch_size = 2
        cls_scores_list = [cls_scores[i] for i in range(num_imgs)] # 2 * (900, 10)
        bbox_preds_list = [bbox_preds[i] for i in range(num_imgs)] # 2 * (900, 10)
        ...
        # labels_list           -> 2 * (900,)
        # label_weights_list    -> 2 * (900,)
        # bbox_targets_list     -> 2 * (900, 9)
        # bbox_weights_list     -> 2 * (900, 10)
        # num_total_pos  45
        # num_total_neg  1755   neg : pos = 39 : 1

        # 将上面的信息整合到一个tensor中
        labels = torch.cat(labels_list, 0)   # (2*900, )
        label_weights = torch.cat(label_weights_list, 0)  # (2*900, )
        bbox_targets = torch.cat(bbox_targets_list, 0)    # (2*900, 9)
        bbox_weights = torch.cat(bbox_weights_list, 0)    # (2*900, 10)
        ...
        if self.sync_cls_avg_factor: # False
            cls_avg_factor = reduce_mean(
                cls_scores.new_tensor([cls_avg_factor]))
        cls_avg_factor = max(cls_avg_factor, 1) # 45
        loss_cls = self.loss_cls(   # cls_scores -> (2*900, 10)   //  labels -> (2*900, )  //  label_weights -> [1] * 1800
        ...
        bbox_preds = bbox_preds.reshape(-1, bbox_preds.size(-1))   # (2, 900, 10)
        # GT本来是9-dim [cx, cy, cz, w, l, h, rot, vx, vy]， 将其转化成10-dim [cx, cy, cz, w, l, h, rot.sin(), rot.cos(), vx, vy]
        normalized_bbox_targets = normalize_bbox(bbox_targets, self.pc_range) # self.pc_range没用
        isnotnan = torch.isfinite(normalized_bbox_targets).all(dim=-1)
        bbox_weights = bbox_weights * self.code_weights

        loss_bbox = self.loss_bbox(
                bbox_preds[isnotnan, :10], normalized_bbox_targets[isnotnan, :10], bbox_weights[isnotnan, :10], avg_factor=num_total_pos)
        # loss训练 点云的图像维度 10-dim [cx, cy, cz, w, l, h, rot.sin(), rot.cos(), vx, vy]
    ...
    def loss(self,
        ...
        all_cls_scores = preds_dicts['all_cls_scores']  # (cam_num, batch_size, 900, 10) -> (6, 2, 900, 10)
        all_bbox_preds = preds_dicts['all_bbox_preds']

        num_dec_layers = len(all_cls_scores)  # cam_num 6
        device = gt_labels_list[0].device
        gt_bboxes_list = [torch.cat(
            (gt_bboxes.gravity_center, gt_bboxes.tensor[:, 3:]),
            dim=1).to(device) for gt_bboxes in gt_bboxes_list]

        # all_gt_bboxes_list = (cam, batch_size, num_obj, 9) // all_gt_bboxes_list[0][0] = (6, 2, (23, 9))
        all_gt_bboxes_list = [gt_bboxes_list for _ in range(num_dec_layers)]
        # all_gt_labels_list = (cam, batch_size, num_obj, 1) // all_gt_labels_list[0][0] = (6, 2, (23, ))
        all_gt_labels_list = [gt_labels_list for _ in range(num_dec_layers)]
        # all_gt_bboxes_ignore_list [None] * cam6
        all_gt_bboxes_ignore_list = [
            gt_bboxes_ignore for _ in range(num_dec_layers)
        ]

        # 通过多次调用 self.loss_single 来计算每一个cam下的损失
