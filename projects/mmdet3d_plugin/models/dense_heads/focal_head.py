...
class FocalHead(AnchorFreeHead):
    ...
    def forward(self, location, **data):
        src = data['img_feats'] # (2, 6, 256, 16, 44)
        bs, n, c, h, w= src.shape
        num_tokens = n*h*w # 6*16*44=4224
        
        # focal sampling
        if self.training: # True
            if self.use_hybrid_tokens: # False
                sample_ratio = random.uniform(0.2, 1.0)
            else:
                sample_ratio = self.train_ratio # 1.0
            num_sample_tokens = int(num_tokens * sample_ratio) # 4244*1.0
           
        else:
            sample_ratio = self.infer_ratio
            num_sample_tokens = int(num_tokens * sample_ratio)

        ##### 将img_fearture转化成token形式的信息
        x = src.flatten(0, 1) # (12, 256, 16, 44)
        cls_feat = self.shared_cls(x)
        cls = self.cls(cls_feat)
        # 分类头 cls -> (12, 10, 16, 44)
        centerness = self.centerness(cls_feat)
        # 置信度头 -> (12, 1, 16, 44) 判断是否是关键位置

        # 一共有16*44=704个token
        # 分类得分与中心度
        cls_logits = cls.permute(0,2,3,1).reshape(bs*n,-1,self.num_classes) # (12, 704, 10)
        centerness = centerness.permute(0,2,3,1).reshape(bs*n,-1,1) # (12, 704, 1)
        pred_bboxes = None
        pred_centers2d = None

        # 通过feature map计算center offset 以及 ltrb(left, top, right, bottom)
        reg_feat = self.shared_reg(x) # (12, 256, 16, 44)
        ltrb = self.ltrb(reg_feat).permute(0,2,3,1).contiguous()
        ltrb = ltrb.sigmoid()   # (12, 16, 44, 4)
        centers2d_offset = self.center2d(reg_feat).permute(0,2,3,1).contiguous() # (12, 16, 44, 2)

        # 根据feature_map在原图中对应的位置location与offset，得到中心点位置
        centers2d = apply_center_offset(location, centers2d_offset)  # (12, 16, 44, 2)
        # 根据feature_map在原图中对应的位置location与ltrb(left, top, right, bottom)得到bbox
        bboxes = apply_ltrb(location, ltrb)

        pred_bboxes = bboxes.view(bs*n,-1,4)
        pred_centers2d = centers2d.view(bs*n,-1,2)

        # 只使用cls_logits中的第一名来作为存在物体的置信度
        ...
        # sample_weight 来决定采样的重要程度，即采样的优先顺序

        _, topk_indexes = torch.topk(sample_weight, num_sample_tokens, dim=1)  # 取样的数量为6*16*44*1.0 = 4224*1.0


        outs = {
                'enc_cls_scores': cls_logits,       # (12, 704, 10)
                'enc_bbox_preds': pred_bboxes,      # (12, 704, 4)
                'pred_centers2d': pred_centers2d,   # (12, 704, 2)
                'centerness':centerness,            # (12, 704, 1)
                'topk_indexes':topk_indexes,        # (2, 4224, 1)  根据sample_weight决定的index顺序，sample_weight = cls_score * centerness
            }
