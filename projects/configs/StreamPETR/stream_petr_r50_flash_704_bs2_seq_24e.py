...
num_gpus = 4
batch_size = 2
num_iters_per_epoch = 28130 // (num_gpus * batch_size) # 28130 = all data sum
num_epochs = 24
...
model = dict(
    ...
    img_backbone=dict(
        ...
        out_indices=(2, 3),
        frozen_stages=-1,
        norm_cfg=dict(type='BN2d', requires_grad=False), #  requires_grad=False
        norm_eval=True,
        with_cp=True,
        style='pytorch'),
    img_neck=dict(
...
# evaluation = dict(interval=num_iters_per_epoch*2, pipeline=test_pipeline)
